...
export function ReviewView({ projectFiles, onDeleteFiles }: ReviewViewProps) {
  ...
  const fetchFiles = async () => {
    ...
    try {
      ...
      // Don't apply filterFileType since we're hardcoding 'Findings' above
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="relative">
              ...
              {showSortMenu && (
                <>
                  ...
                  <div 
                    className="fixed inset-y-0 left-0 right-[350px] z-40"
      ...
      {/* File Viewer Modal */}
      {viewingFile && (
        <div className="fixed inset-y-0 left-0 right-[350px] z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
            {/* Modal Header */}
            <div className="border-b-2 border-gray-300 p-4 flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <h3 className="text-gray-900 font-semibold truncate">{viewingFile.fileName}</h3>
                <div className="text-xs text-gray-600 mt-1 space-x-4">
                  <span>Brand: {viewingFile.brand || 'N/A'}</span>
                  <span>Project: {viewingFile.projectType || 'N/A'}</span>
                  <span>Type: {viewingFile.fileType}</span>
                  <span>Date: {new Date(viewingFile.uploadDate).toLocaleDateString()}</span>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {loadingContent ? (
                ...
                <pre className="whitespace-pre-wrap font-mono text-sm text-gray-900 bg-gray-50 p-4 rounded">
                  {fileContent}
                </pre>
              )}
            </div>
            ...
            <div className="border-t-2 border-gray-300 p-4 flex items-center justify-end gap-2">
              <button
                onClick={async () => {
                  await downloadKnowledgeBaseFile(viewingFile.fileId, viewingFile.fileName);
                }}
