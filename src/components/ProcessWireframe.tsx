...
export default function ProcessWireframe() {
  ...
  const loadSharedConfig = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Trust Databricks as the authoritative source — always overwrite localStorage
        // so all workspace users see the same list regardless of local state.
        // Only fall back to localStorage if the fetch itself failed (network error etc.)
        const brands = result.brands || [];
        if (brands.length > 0) {
          setAvailableBrands(brands);
          localStorage.setItem('cohive_available_brands', JSON.stringify(brands));
        } else {
          // Databricks returned empty — could be a fresh workspace with no brands yet.
          // Keep localStorage brands so the UI isn't blank, but don't override with them.
        ...
        // Fetch failed entirely — fall back to localStorage
        const savedBrands = localStorage.getItem('cohive_available_brands');
        if (savedBrands) { setAvailableBrands(JSON.parse(savedBrands)); }
  ...
  useEffect(() => {
    const fetchUserEmail = async () => {
      if (isDatabricksAuthenticated) {
        ...
        // the on-mount call may have run before auth was established
        // and returned an empty list, falling back to stale localStorage.
  ...
  useEffect(() => {
    if (activeStepId === 'Enter') {
      ...
      if (brand && projectType) {
        ...
        if (!currentFilename || (isAutoGenerated && !currentFilename.includes(`_${brand}_${projectType}_`))) {
            ...
            <div>
             ...
             <div className="space-y-4">
                {activeStepId === 'research' ? (
                  // All users can read and download KB files for their selected brand.
                  // canApproveResearch controls whether approve/reject actions are shown —
                  // only privileged roles get that capability.
