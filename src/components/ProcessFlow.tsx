...
export function ProcessFlow({
  ...
  const isStepClickable = (stepId: string): boolean => {
    ...
    // Grade (Score Results) and Findings require at least one center hex to be executed
    if (stepId === "Grade" || stepId === "Findings") {
      ...
      // Check if any center hex has been executed in this iteration
      const hasCenterHexExecution = centerHexes.some(
        hexId => hexExecutions[hexId]?.length > 0
      );
      
      if (!hasCenterHexExecution) {
        return false;
      }
    }

    // After Enter is complete, all steps become clickable (except Grade/Findings which have their own check above)
    if (isEnterComplete) {
      return true;
    }
