...
interface DatabricksFileBrowserProps {
  ...
  userRole?: 'researcher' | 'non-researcher'; // Optional: to enforce approval restrictions
}

export function DatabricksFileBrowser({ open, onClose, onFilesSelected, userRole = 'non-researcher' }: DatabricksFileBrowserProps) {
  ...
  const handleImportSelected = async () => {
    ...
    try {
      ...
      if (viewMode === 'knowledge-base') {
        ...
        // Research leaders must approve files first before they can be used
        ...
        if (unapprovedFiles.length > 0) {
          ...
          setError(`Cannot use unapproved files: ${fileNames}. Files must be approved in the Knowledge Base before they can be used in hexes.`);
          setImporting(false);
          return;
        }
          ...
          {viewMode === 'knowledge-base' && (
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm" style={{ color: colors.text.secondary }}>
                📚 All users can view all files. Only approved files can be used in hexes.
              </p>
              <Button
                variant="outline"
                size="sm"
