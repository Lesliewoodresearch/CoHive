/**
 * Panel Homes (Stories) Hex Prompts
 *
 ...
 * The hex framing (Purpose/Style/Approach) is injected via getHexFraming('stories').
 */
...
export const PanelHomesPrompts = {
  ...
  executeAssess: new PromptTemplate({
    ...
    parts: [
      new TextPart('header', {
        en: '=== STORIES — HOUSEHOLD ASSESSMENT ==='
      }),

      new TextPart('purpose', {
        en: `**Session Purpose:**
You are a stories evaluating this work through the lens of your specific household — how this brand, product, or strategy actually fits into the rhythms of domestic life.
...
- No complimenting other storiess unless a specific point directly matches your household experience
...
Assessment Instructions for ${personaCount} stories persona${personaCount !== 1 ? 's' : ''}:
...
- Note where household demographics and lifestyle create specific needs or barriers`;
  ...
  executeRecommend: new PromptTemplate({
    ...
    parts: [
      new TextPart('header', {
        en: '=== STORIES — HOUSEHOLD RECOMMENDATIONS ==='
      }),
...
- Where storiess' households differ, preserve those differences — they represent real market segmentation
...
- Family decision-making considerations that the current approach ignores`
  ...
  executeUnified: new PromptTemplate({
    ...
    parts: [
      new TextPart('header', {
        en: '=== STORIES — UNIFIED HOUSEHOLD ANALYSIS ==='
      }),
...
- Preserve the differences between stories households — they show where strategies need to vary
...
3. SYNTHESISE (Round 2+ only)
   - Compare your household reality directly with other storiess
...
The report should reflect the specific household realities surfaced — not generalised consumer behaviour but the concrete domestic contexts, routines, and decision dynamics revealed through stories perspectives.`
      }),
...
- Household Profiles: Each stories's context, behaviour patterns, and key findings
...
- Cross-Household Patterns: Where all storiess agree vs where they diverge
...
Metadata:
- Date, project, brand, stories personas: ${ctx.selectedPersonas?.join(', ') || 'None'}
- KB files analysed: ${ctx.selectedFiles?.join(', ') || 'None'}

Content:
- Full household assessments by stories
