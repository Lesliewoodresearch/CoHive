...
// Increase body size limit to handle large file uploads (up to ~37MB pre-encoding → ~50MB base64)
export const config = {
  api: {
    bodyParser: {
      sizeLimit: '50mb',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Guard against files that slipped past the client-side check
    const estimatedBytes = Math.ceil((fileContent.length * 3) / 4);
    const MAX_BYTES = 37 * 1024 * 1024; // 37MB
    if (estimatedBytes > MAX_BYTES) {
      return res.status(413).json({
        error: 'File too large',
        message: `File exceeds the 37MB limit (estimated ${Math.round(estimatedBytes / 1024 / 1024)}MB). Please compress the file before uploading.`,
      });
    }
    ...
    const errMsg = error instanceof Error
      ? (error.message || error.toString() || 'Unknown server error')
      : String(error);
    console.error('[KB Upload] Error:', errMsg, error);
    try {
      logError({
    ...
    } catch (_) { /* logger failure should not mask original error */ }
    return res.status(500).json({ error: 'Upload failed', message: errMsg });
  }
}
